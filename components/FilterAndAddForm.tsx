...
export const FilterAndAddForm: React.FC<AddFormProps> = ({ onAddRow, isOnline }) => {
  const [supplier, setSupplier] = useState(SUPPLIERS[0]);
  const [pallets, setPallets] = useState<number | ''>('');

  const canSubmit = pallets !== '' && Number(pallets) > 0 && isOnline;
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow space-y-4">
      ...
      >
        ...
        {isOnline ? 'Προσθήκη' : 'Εκτός Σύνδεσης'}
      </button>
    </form>
  );
};